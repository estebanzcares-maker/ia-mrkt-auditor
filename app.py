            ...
            if err: st.error(err); st.session_state["run_audit"]=False
                ...
                with c2:
                    def gen_pdf():
                        ...
                        c.setFillColor(HexColor("#FF3B30") if total_fuga>0 else HexColor("#CCFF00")); c.setFont("Helvetica-Bold", 26); c.drawString(M+10, H-128, f"${total_fuga:,.0f} CLP/mes")
                        c.setFont("Helvetica", 9); c.setFillColor(HexColor("#FFFFFF")); c.drawString(M+10, H-145, "Fuga estimada mensual")
                        c.setFillColor(HexColor("#FFFFFF")); c.setFont("Helvetica-Bold", 13); c.drawString(M+220, H-128, f"{total_camp} campañas | {num_rojos} críticas | {num_verdes} verdes")
                        c.setFont("Helvetica", 8); c.setFillColor(HexColor("#AAAAAA")); c.drawString(M+220, H-142, f"Plataforma {plat_usar} • Recupero 35-80% • ROI x4.2")
                        y=H-190
                        ...
                        p=Paragraph(resumen, style_small_w); _,ph=p.wrap(W-2*M, 80); p.drawOn(c, M, y-ph); y-=ph+18
                        box_w=(W-2*M-10)/2
                        ...
                        _,h_c=p_c.wrap(box_w-20, 200); _,h_i=p_i.wrap(box_w-20, 200)
                        box_h=max(h_c,h_i)+24
                        ...
                        p_c.drawOn(c, M+10, y-box_h+12); p_i.drawOn(c, M+box_w+20, y-box_h+12)
                        y-=box_h+20
                        ...
                        p4=Paragraph(metod, style_tiny); _,hm=p4.wrap(W-2*M, 60); p4.drawOn(c, M, y-hm)
                        footer(1); c.showPage()
                        fondo_negro()
                        ...
                        p_anal=Paragraph(analisis, style_small_w); _,ha=p_anal.wrap(W-2*M, 70); p_anal.drawOn(c, M, y2-ha); y2-=ha+18
                        ...
                        for a in res['rojos'][:3]:
                            if y3<180: footer(3); c.showPage(); fondo_negro(); y3=H-55
                    ...
                    try:
                        ...
                        if enviado: st.markdown(f'<div style="padding:10px; background:#0A1C0A; border:1px solid #CCFF00; border-radius:10px; color:#CCFF00;">✅ PDF enviado a {email_final}</div>', unsafe_allow_html=True)
                            ...
                            if "no configurado" not in det.lower(): st.markdown(f'<div style="font-size:11px; color:#888;">ℹ Email: {det} — Descarga directa habilitada con botón lime</div>', unsafe_allow_html=True)
                    except: pass
