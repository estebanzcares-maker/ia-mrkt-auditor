...
def enviar_pdf_por_email(email_destino, pdf_path, plataforma, fuga_total):
    try:
        ...
        params = {
            ...
            "html": f"<h2>Tu auditoría IA.MRKT {plataforma} está lista</h2><p><strong>Fuga detectada:</strong> ${fuga_total:,.0f} CLP/mes</p><p><strong>Email auditado:</strong> {email_destino}</p><p>Adjunto el PDF completo con el detalle de campañas en ROJO y VERDE.</p>",
...
with col2:
    ...
    if st.session_state["auditorias_hechas"] >= LIMITE_CAMPANAS:
        ...
        if csv_file:
            try:
                ...
                if plat_detectada!= plataforma:
                    st.markdown(f'<div style="background:#1C1A0A; border:1px solid #FFAA00; border-left:4px solid #FFAA00; border-radius:8px; padding:10px; font-size:11px; margin-top:8px; color:#E5E5E5;"><span style="color:#FFAA00; font-weight:700;">⚠ AUTODETECT:</span> Detecté CSV de <b style="color:#FFFFFF;">{plat_detectada}</b> pero tenías <b>{plataforma}</b>. Al auditar cambiaré a {plat_detectada} automáticamente.</div>', unsafe_allow_html=True)
        ...
        if btn_auditar:
            if not email:
                st.markdown('<div style="background:#1C0A0A; border:1px solid #FF3B30; border-radius:8px; padding:10px; font-size:11px; color:#E5E5E5; margin-top:8px;"><span style="color:#FF3B30;">⚠ Debes ingresar tu correo para generar el PDF.</span></div>', unsafe_allow_html=True)
    ...
    st.markdown('''<div style="margin-top:6px; font-size:10px; color:#444; text-align:center;">Pago por transferencia • Link privado.</div></div>''', unsafe_allow_html=True)
...
with g1: st.markdown('<div class="gallery-item"><div class="mono" style="font-size:11px; color:#888;">GOOGLE • HOTEL COSTA • $1.2M</div><div class="mono" style="font-weight:700; color:#FF3B30; margin-top:6px;">$602.500 FUGA</div><div style="margin-top:8px; display:flex; gap:4px;"><div style="width:100%; height:6px; background:#FF3B30; border-radius:99px;"></div><div style="width:60%; height:6px; background:#333; border-radius:99px;"></div></div><div style="font-size:10px; color:#666; margin-top:8px;">3 ROJOS • 4 VERDES</div></div>', unsafe_allow_html=True)
with g2: st.markdown('<div class="gallery-item"><div class="mono" style="font-size:11px; color:#888;">META • FERRETERÍA 3 LOCALES • $890k</div><div class="mono" style="font-weight:700; color:#FF3B30; margin-top:6px;">$445.000 FUGA</div><div style="margin-top:8px; display:flex; gap:4px;"><div style="width:70%; height:6px; background:#FF3B30; border-radius:99px;"></div><div style="width:30%; height:6px; background:#333; border-radius:99px;"></div></div><div style="font-size:10px; color:#666; margin-top:8px;">2 AMARILLOS • 2 VERDES</div></div>', unsafe_allow_html=True)
with g3: st.markdown('<div class="gallery-item"><div class="mono" style="font-size:11px; color:#888;">LINKEDIN • SAAS B2B • $750k</div><div class="mono" style="font-weight:700; color:#FFAA00; margin-top:6px;">$407.500 FUGA</div><div style="margin-top:8px; display:flex; gap:4px;"><div style="width:50%; height:6px; background:#FFAA00; border-radius:99px;"></div><div style="width:50%; height:6px; background:#333; border-radius:99px;"></div></div><div style="font-size:10px; color:#666; margin-top:8px;">1 ROJO • 1 AMARILLO</div></div>', unsafe_allow_html=True)
with g4: st.markdown('<div class="gallery-item" style="border-color:#CCFF00;"><div class="mono" style="font-size:11px; color:#CCFF00;">CLÍNICA DENTAL • META • $2.1M</div><div class="mono" style="font-weight:700; color:#CCFF00; margin-top:6px;">$0 FUGA • OPTIMIZADA</div><div style="margin-top:8px; display:flex; gap:4px;"><div style="width:100%; height:6px; background:#CCFF00; border-radius:99px;"></div></div><div style="font-size:10px; color:#666; margin-top:8px;">6 VERDES • ROAS 5.8</div></div>', unsafe_allow_html=True)
    ...
    if not ok:
        ...
        try:
            ...
            if num_camp_csv > LIMITE_CAMPANAS:
                ...
                st.markdown(f'''<div class="alert-amarillo-contraste"><div style="color:#FFAA00; font-weight:700; font-size:12px; margin-bottom:4px;">⚠ LÍMITE DE PLAN</div><div style="color:#E5E5E5; font-size:13px;">Tu CSV tiene <b style="color:#FFFFFF;">{num_camp_csv} campañas</b>, pero tu plan de <b style="color:#CCFF00;">${PRECIO_PLAN:,.0f}</b> incluye solo <b style="color:#FFFFFF;">{LIMITE_CAMPANAS}</b>. Auditaremos solo las primeras {LIMITE_CAMPANAS}.</div><div style="color:#888; font-size:11px; margin-top:6px;">Pack {num_camp_csv} campañas = <b style="color:#CCFF00;">${precio_full:,.0f}</b> • Contacta para pack extra</div></div>''', unsafe_allow_html=True)
            ...
            if err:
                ...
                guardado = guardar_lead(email=email_final, plataforma=plat_usar, fuga=res["total_fuga"], plan_camp=LIMITE_CAMPANAS, precio_plan=PRECIO_PLAN, num_camp_csv=num_camp_csv, num_verdes=len(res["verdes"]), num_rojos=len(res["rojos"]), csv_nombre=csv_nombre)
                ...
                with c2:
                    def gen_pdf():
                        # === PDF V2 PULIDO - SOLO ESTA FUNCION CAMBIA - UI INTACTA ===
                        ...
                        def footer(num):
                            c.setFillColor(HexColor("#444444")); c.setFont("Helvetica", 7); c.drawString(40, 20, f"IA.MRKT • {plat_usar} • {email_final} • {datetime.now().strftime('%d/%m/%Y %H:%M')} • Pag {num}"); c.drawRightString(W-40, 20, "Confidencial")
                        fondo_negro()
                        ...
                        c.setFont("Helvetica", 9); c.setFillColor(HexColor("#888888")); c.drawString(40, H-105, f"Cliente: {email_final} | {datetime.now().strftime('%d/%m/%Y')} | PDF Pulido")
                        ...
                        c.setFont("Helvetica", 10); c.setFillColor(HexColor("#FFFFFF")); c.drawString(55, H-185, "Fuga estimada mensual")
                        ...
                        texto = f"Se analizaron {total_camp} campañas de {plat_usar}. " + (f"Fuga ${total_fuga:,.0f}/mes en {num_rojos} campañas. Recuperable 35-80%." if total_fuga>0 else f"{num_verdes} campañas optimizadas.")
                        ...
                        for b in ["1. Tabla completa campañas + métricas reales", "2. Alertas ROJA/AMARILLA con acción", "3. Verdes para escalar", "4. Metodología + próximos pasos"]:
                            c.drawString(45, y, f"• {b}"); y-=12
                        footer(1); c.showPage()
                        fondo_negro(); c.setFillColor(HexColor("#CCFF00")); c.setFont("Helvetica-Bold", 14); c.drawString(40, H-50, f"Detalle Completo — {total_camp} campañas [{plat_usar}]")
                        ...
                        fondo_negro(); c.setFillColor(HexColor("#FF3B30")); c.setFont("Helvetica-Bold", 14); c.drawString(40, H-50, f"Alertas Críticas — {len(res['rojos'])}"); y=H-80
                        if len(res['rojos'])==0:
                            c.setFillColor(HexColor("#CCFF00")); c.setFont("Helvetica-Bold",12); c.drawString(40,y,"✓ Sin alertas rojas")
                        else:
                            for a in res['rojos'][:8]:
                                ...
                                c.setFillColor(HexColor("#CCFF00")); c.setFont("Helvetica-Bold",7.5); rec="ACCIÓN: Pausar 48h, revisar segmentación" if "0" in a['tipo'] else "ACCIÓN: Cambiar creatividad, bajar freq <2.0"; c.drawString(50,y-38,rec); y-=62
                        footer(3); c.showPage()
                        fondo_negro(); c.setFillColor(HexColor("#CCFF00")); c.setFont("Helvetica-Bold",14); c.drawString(40,H-50,f"Verdes — {len(res['verdes'])}"); y=H-80
                    ...
                    try:
                        ...
                        if enviado:
                            ...
                            if "no configurado" not in detalle_envio.lower():
                                st.markdown(f'<div style="margin-bottom:10px; font-size:11px; color:#888;">ℹ Email automático: {detalle_envio}</div>', unsafe_allow_html=True)
                    except Exception as e:
                        pass
                    with open(pdf_path,"rb") as f:
